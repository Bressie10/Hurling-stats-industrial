-- Tighten account deletion RPC auth handling after the initial function was
-- already applied in some environments.
--
-- Supabase only applies each migration once, so this follow-up migration
-- redefines the function with an explicit role guard. Anonymous callers should
-- fail instead of receiving a successful no-op response.
